...
class _EventEditorSheetState extends State<_EventEditorSheet> {
  ...
  @override
  void dispose() { _title.dispose(); _location.dispose(); _note.dispose(); super.dispose(); }
  ...
  Future<void> _pickDate(bool start) async {
    ...
    setState(() {
      ...
      if (start) { _start = value; if (!_end.isAfter(_start)) _end = _start.add(const Duration(hours: 1)); } else { _end = value; }
    });
  }
  ...
  Future<void> _pickTime(bool start) async {
    ...
    await showModalBottomSheet<void>(context: context, showDragHandle: true, backgroundColor: Theme.of(context).colorScheme.surface, builder: (sheetContext) => SafeArea(child: SizedBox(height: 330, child: Column(children: [
...
class _ReminderDialogState extends State<_ReminderDialog> {
  ...
  @override Widget build(BuildContext context) => AlertDialog(scrollable: true, title: const Text('Báo trước'), content: Column(mainAxisSize: MainAxisSize.min, children: [SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Báo trước'), value: _enabled, onChanged: (v) => setState(() => _enabled = v)), const SizedBox(height: 4), _NumberSetting(label: 'Báo trước', suffix: 'phút', controller: _minutes, enabled: _enabled, autofocus: _enabled), const SizedBox(height: 12), _NumberSetting(label: 'Lặp lại nếu chưa xác nhận', suffix: 'lần', controller: _repeatCount, enabled: _enabled), const SizedBox(height: 12), _NumberSetting(label: 'Khoảng cách giữa 2 lần', suffix: 'phút', controller: _interval, enabled: _enabled)]), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Hủy')), FilledButton(onPressed: _save, child: const Text('Lưu'))]);
}
...
class _EventContent extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final s = Theme.of(context).colorScheme; final bottomInset = MediaQuery.viewInsetsOf(context).bottom; return Container(width: double.infinity, decoration: BoxDecoration(color: s.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(42))), child: Column(children: [Expanded(child: SingleChildScrollView(keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag, padding: EdgeInsets.fromLTRB(28, 26, 28, 110 + bottomInset), child: Column(children: [_TitleRow(controller: title, priority: priority, onPriority: onPriority), const _Divider(), const SizedBox(height: 28), _DateTimeRange(start: start, end: end, onStartDate: onStartDate, onStartTime: onStartTime, onEndDate: onEndDate, onEndTime: onEndTime), const _Divider(), _InputRow(icon: Icons.location_on_outlined, controller: location, hint: 'Địa điểm'), const _Divider(), _InputRow(icon: Icons.notes_outlined, controller: note, hint: 'Ghi chú', maxLines: 3), const _Divider(), _ActionRow(icon: Icons.notifications_none_outlined, title: reminderMinutes == 0 ? 'Không báo trước' : 'Trước $reminderMinutes phút', onTap: onReminder), const _Divider(), _ActionRow(icon: Icons.notification_important_outlined, title: reminderMinutes == 0 ? 'Không lặp thông báo' : 'Lặp $reminderRepeatCount lần · mỗi $reminderRepeatIntervalMinutes phút', onTap: onReminder, muted: reminderMinutes == 0), const _Divider(), _ActionRow(icon: Icons.repeat_rounded, title: _recurrenceName(recurrence), onTap: onRecurrence, muted: recurrence == RecurrenceFrequency.none)]))), _SaveBar(onCancel: onCancel, onSave: onSave)])); }
}
...
class _BulkImportSlot extends StatelessWidget { const _BulkImportSlot(); @override Widget build(BuildContext context) { final s = Theme.of(context).colorScheme; return Container(width: double.infinity, decoration: BoxDecoration(color: s.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(42))), child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.auto_awesome_outlined, size: 42, color: s.primary), const SizedBox(height: 14), const Text('AI Import', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)), const SizedBox(height: 6), Text('Nhập lịch hàng loạt với sự hỗ trợ của AI.', style: TextStyle(color: s.onSurfaceVariant))]))); } }
class _TitleRow extends StatelessWidget { const _TitleRow({required this.controller, required this.priority, required this.onPriority}); final TextEditingController controller; final int priority; final VoidCallback onPriority; @override Widget build(BuildContext context) => Row(children: [const Icon(Icons.emoji_emotions_outlined, size: 30), const SizedBox(width: 14), Expanded(child: TextField(controller: controller, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w400), decoration: const InputDecoration(hintText: 'Tựa đề', border: InputBorder.none, isDense: true))), GestureDetector(onTap: onPriority, child: _PriorityDot(priority: priority, size: 40))]); }
class _DateTimeRange extends StatelessWidget { const _DateTimeRange({required this.start, required this.end, required this.onStartDate, required this.onStartTime, required this.onEndDate, required this.onEndTime}); final DateTime start, end; final VoidCallback onStartDate, onStartTime, onEndDate, onEndTime; String _date(DateTime d) => 'T.${d.weekday == 7 ? 'CN' : d.weekday + 1}, ${d.day.toString().padLeft(2, '0')}/${d.month.toString().padLeft(2, '0')}'; String _time(DateTime d) { final h = d.hour % 12 == 0 ? 12 : d.hour % 12; return '${h.toString().padLeft(2, '0')}:${d.minute.toString().padLeft(2, '0')} ${d.hour >= 12 ? 'PM' : 'AM'}'; } @override Widget build(BuildContext context) => Row(children: [Expanded(child: _DateTimeColumn(date: _date(start), time: _time(start), onDate: onStartDate, onTime: onStartTime)), const Padding(padding: EdgeInsets.symmetric(horizontal: 12), child: Icon(Icons.arrow_forward_rounded, size: 32)), Expanded(child: _DateTimeColumn(date: _date(end), time: _time(end), onDate: onEndDate, onTime: onEndTime))]); }
...
class _InputRow extends StatelessWidget { const _InputRow({required this.icon, required this.controller, required this.hint, this.maxLines = 1}); final IconData icon; final TextEditingController controller; final String hint; final int maxLines; @override Widget build(BuildContext context) => Row(crossAxisAlignment: maxLines > 1 ? CrossAxisAlignment.start : CrossAxisAlignment.center, children: [SizedBox(width: 40, child: Padding(padding: EdgeInsets.only(top: maxLines > 1 ? 10 : 0), child: Icon(icon))), const SizedBox(width: 14), Expanded(child: TextField(controller: controller, maxLines: maxLines, minLines: maxLines > 1 ? 2 : 1, style: const TextStyle(fontSize: 17), decoration: InputDecoration(hintText: hint, border: InputBorder.none, isDense: true)))]); }
class _ActionRow extends StatelessWidget { const _ActionRow({required this.icon, required this.title, required this.onTap, this.muted = false}); final IconData icon; final String title; final VoidCallback onTap; final bool muted; @override Widget build(BuildContext context) { final s = Theme.of(context).colorScheme; return InkWell(onTap: onTap, child: Padding(padding: const EdgeInsets.symmetric(vertical: 15), child: Row(children: [SizedBox(width: 40, child: Icon(icon)), const SizedBox(width: 14), Expanded(child: Text(title, style: TextStyle(fontSize: 17, color: muted ? s.onSurfaceVariant : s.onSurface))), Icon(Icons.chevron_right_rounded, color: s.onSurfaceVariant)]))); } }
class _Divider extends StatelessWidget { const _Divider(); @override Widget build(BuildContext context) => Divider(height: 1, color: Theme.of(context).colorScheme.outlineVariant); }
...
Color _priorityColor(BuildContext context, int priority) { final s = Theme.of(context).colorScheme; return switch (priority) { 0 => s.primary, 1 => s.tertiary, _ => s.error }; }
String _priorityName(int priority) => switch (priority) { 0 => 'Bình thường', 1 => 'Quan trọng', _ => 'Rất quan trọng' };
...
String _recurrenceName(RecurrenceFrequency frequency) => switch (frequency) { RecurrenceFrequency.none => 'Không lặp lại', RecurrenceFrequency.daily => 'Hàng ngày', RecurrenceFrequency.weekly => 'Hàng tuần', RecurrenceFrequency.weekdays => 'Thứ 2 – Thứ 6', RecurrenceFrequency.monthly => 'Hàng tháng' };
IconData _recurrenceIcon(RecurrenceFrequency frequency) => switch (frequency) { RecurrenceFrequency.none => Icons.repeat_rounded, RecurrenceFrequency.daily => Icons.today_outlined, RecurrenceFrequency.weekly => Icons.view_week_outlined, RecurrenceFrequency.weekdays => Icons.work_outline_rounded, RecurrenceFrequency.monthly => Icons.calendar_month_outlined };
