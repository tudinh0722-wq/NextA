...
class _EventEditorSheetState extends State<_EventEditorSheet> {
  ...
  Future<void> _pickTime(bool isStart) async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (c) => SafeArea(child: SizedBox(height: 300, child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(24, 4, 24, 4), child: Row(children: [
        ...
        Expanded(child: CupertinoDatePicker(mode: CupertinoDatePickerMode.time, use24hFormat: false, initialDateTime: current, onDateTimeChanged: (v) {
      ...
      builder: (c) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          for (final f in RecurrenceFrequency.values)
            ListTile(contentPadding: EdgeInsets.zero, leading: Icon(_recurrenceIcon(f)), title: Text(_recurrenceName(f)), trailing: f == _recurrenceRule.frequency ? const Icon(Icons.check_rounded) : null, onTap: () => Navigator.pop(c, f)),
        ])),
      ),
    );
  ...
  Future<void> _pickPriority() async {
    final value = await showModalBottomSheet<int>(
      ...
      showDragHandle: true,
      builder: (c) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        for (final p in [0, 1, 2]) ListTile(leading: _PriorityDot(priority: p), title: Text(_priorityName(p)), trailing: p == _priority ? const Icon(Icons.check_rounded) : null, onTap: () => Navigator.pop(c, p)),
        const SizedBox(height: 8),
      ])),
    );
  ...
  void _save() {
    ...
    final event = NextAEvent(
      id: id, title: title, type: old?.type ?? EventType.classEvent, start: _start, end: _end,
      location: _location.text.trim().isEmpty ? null : _location.text.trim(),
      note: _note.text.trim().isEmpty ? null : _note.text.trim(),
      priority: _priority, recurrenceId: recurring ? (old?.recurrenceId ?? id) : old?.recurrenceId,
      recurrenceRule: recurring ? _recurrenceRule : null,
      reminderMinutes: _reminderMinutes, reminderRepeatCount: _reminderRepeatCount, reminderRepeatIntervalMinutes: _reminderRepeatIntervalMinutes,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Material(color: s.surfaceContainerLowest, child: SafeArea(top: true, bottom: false, child: Column(children: [
      ...
      Expanded(child: _bulkImportTab ? const _BulkImportSlot() : _EventContent(
        title: _title, location: _location, note: _note, start: _start, end: _end, priority: _priority,
        reminderMinutes: _reminderMinutes, reminderRepeatCount: _reminderRepeatCount, reminderRepeatIntervalMinutes: _reminderRepeatIntervalMinutes,
        recurrence: _recurrenceRule.frequency, onPriority: _pickPriority, onStartDate: () => _pickDate(true), onStartTime: () => _pickTime(true), onEndDate: () => _pickDate(false), onEndTime: () => _pickTime(false), onReminder: _pickReminder, onRecurrence: _pickRecurrence, onCancel: () => Navigator.pop(context), onSave: _save,
      )),
    ])));
  }
}
...
class _ReminderDialog extends StatefulWidget {
  ...
  final int minutes, repeatCount, interval;
  @override State<_ReminderDialog> createState() => _ReminderDialogState();
}

class _ReminderDialogState extends State<_ReminderDialog> {
  late final TextEditingController _minutes, _repeatCount, _interval;
  late bool _enabled;
  @override void initState() {
    ...
    _interval = TextEditingController(text: widget.interval.toString());
  }
  @override void dispose() { _minutes.dispose(); _repeatCount.dispose(); _interval.dispose(); super.dispose(); }
  int _value(TextEditingController c, int fallback, int min, int max) => (int.tryParse(c.text.trim()) ?? fallback).clamp(min, max);
  void _save() => Navigator.pop(context, _ReminderSettings(enabled: _enabled, minutes: _value(_minutes, 10, 1, 10080), repeatCount: _value(_repeatCount, 2, 0, 20), interval: _value(_interval, 5, 1, 1440)));
...
class _NumberSetting extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Row(children: [
      ...
      SizedBox(width: 64, child: TextField(controller: controller, autofocus: autofocus, enabled: enabled, textAlign: TextAlign.center, keyboardType: const TextInputType.numberWithOptions(decimal: false), decoration: const InputDecoration(isDense: true))),
      const SizedBox(width: 8), SizedBox(width: 42, child: Text(suffix)),
    ]);
  }
}
...
class _EventContent extends StatelessWidget {
  ...
  final VoidCallback onPriority, onStartDate, onStartTime, onEndDate, onEndTime, onReminder, onRecurrence, onCancel, onSave;
  @override Widget build(BuildContext context) {
    final s = Theme.of(context).colorScheme;
    final bottomInset = MediaQuery.viewInsetsOf(context).bottom;
    return Container(width: double.infinity, decoration: BoxDecoration(color: s.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(42))), child: Column(children: [
...
class _EditorTabs extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final s = Theme.of(context).colorScheme;
    return Container(height: 48, padding: const EdgeInsets.all(2), decoration: BoxDecoration(color: s.surfaceContainerHighest, borderRadius: BorderRadius.circular(26)), child: Row(children: [
...
class _BulkImportSlot extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final s = Theme.of(context).colorScheme;
    return Container(width: double.infinity, decoration: BoxDecoration(color: s.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(42))), alignment: Alignment.topCenter, padding: const EdgeInsets.fromLTRB(28, 42, 28, 28), child: Column(children: [
      Icon(Icons.auto_awesome_motion_outlined, size: 42, color: s.primary), const SizedBox(height: 14),
      Text('AI Import', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)), const SizedBox(height: 8),
...
class _TitleRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Row(children: [
    Icon(Icons.emoji_emotions_outlined, size: 30, color: Theme.of(context).colorScheme.onSurfaceVariant), const SizedBox(width: 16),
    ...
    const SizedBox(width: 12), InkWell(onTap: onPriority, borderRadius: BorderRadius.circular(22), child: Padding(padding: const EdgeInsets.all(8), child: _PriorityDot(priority: priority, size: 30))),
  ]);
}
...
class _PriorityDot extends StatelessWidget {
  ...
  final int priority;
  final double size;
  @override Widget build(BuildContext context) => Container(width: size, height: size, decoration: BoxDecoration(shape: BoxShape.circle, color: _priorityColor(context, priority)));
}
...
class _InputRow extends StatelessWidget {
  ...
  final String hint;
  final int maxLines;
  @override Widget build(BuildContext context) => Row(crossAxisAlignment: maxLines > 1 ? CrossAxisAlignment.start : CrossAxisAlignment.center, children: [
    Padding(padding: EdgeInsets.only(top: maxLines > 1 ? 10 : 0), child: Icon(icon, size: 30)), const SizedBox(width: 20),
...
class _ActionRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final s = Theme.of(context).colorScheme;
    return InkWell(onTap: onTap, child: Padding(padding: const EdgeInsets.symmetric(vertical: 16), child: Row(children: [
      Icon(icon, size: 30, color: muted ? s.onSurfaceVariant : s.onSurface), const SizedBox(width: 20),
      Expanded(child: Text(title, style: TextStyle(fontSize: 20, color: muted ? s.onSurfaceVariant : s.onSurface))), const SizedBox(width: 12),
...
class _Divider extends StatelessWidget {
  const _Divider();
  @override Widget build(BuildContext context) => Divider(height: 1, color: Theme.of(context).colorScheme.outlineVariant.withValues(alpha: .65));
}
...
class _SaveBar extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final s = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.fromLTRB(28, 8, 28, 18), child: Material(elevation: 2, shadowColor: s.shadow.withValues(alpha: .18), color: s.surfaceContainerLowest, shape: const StadiumBorder(), child: SizedBox(height: 62, child: Row(children: [
...
Color _priorityColor(BuildContext context, int priority) {
  final s = Theme.of(context).colorScheme;
  return switch (priority) { 2 => s.error, 1 => s.tertiary, _ => s.primary };
}
...
String _timeLabel(DateTime v) { final h = v.hour % 12 == 0 ? 12 : v.hour % 12; final m = v.minute.toString().padLeft(2, '0'); return '$h:$m ${v.hour >= 12 ? 'PM' : 'AM'}'; }
