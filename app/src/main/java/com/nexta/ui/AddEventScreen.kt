...
fun AddEventScreen(onBack: () -> Unit, onSave: (Event) -> Unit, initialEvent: Event? = null, onBulkImport: () -> Unit = {}) {
    ...
    fun save() {
        ...
        try {
            ...
            startDateTime = LocalDateTime.of(d, LocalTime.parse(startTime.trim()))
            endDateTime = LocalDateTime.of(d, LocalTime.parse(endTime.trim()))
        } catch (_: Exception) {
    ...
    Scaffold(
        ...
        Column(
            ...
            TextField(
                title,
                { title = it.take(MAX_TITLE_LENGTH); errorMessage = null },
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                TextField(
                    startTime,
                    { startTime = it; errorMessage = null },
                    label = { Text("Bắt đầu") },
            ...
            TextField(
                location,
                { location = it.take(MAX_LOCATION_LENGTH) },
                label = { Text("Địa điểm") },
                modifier = Modifier.fillMaxWidth(),
                singleLine = true
            )
            ...
            if (initialEvent == null) {
                OutlinedButton(onClick = onBulkImport, modifier = Modifier.fillMaxWidth()) {
                    Text("Nhập nhiều sự kiện từ AI")
                }
            }
...
private fun PriorityButton(value: Int, label: String, selected: Int, modifier: Modifier, onClick: () -> Unit) {
    if (value == selected) Button(onClick, modifier) { Text(label) }
