...
fun AddEventScreen(
    ...
    fun save() {
        ...
        try {
            startDateTime = LocalDateTime.of(LocalDate.parse(date.trim()), LocalTime.parse(startTime.trim()))
            endDateTime = LocalDateTime.of(LocalDate.parse(date.trim()), LocalTime.parse(endTime.trim()))
        } catch (_: Exception) { errorMessage = "Ngày hoặc giờ không hợp lệ. Ví dụ: 2026-09-08 và 08:30."; return }
        if (!endDateTime.isAfter(startDateTime)) { errorMessage = "Giờ kết thúc phải sau giờ bắt đầu."; return }
        onSave(Event(initialEvent?.id ?: UUID.randomUUID().toString(), trimmedTitle, type, startDateTime, endDateTime, trimmedLocation, trimmedNote, priority))
    }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(if (initialEvent == null) "Thêm sự kiện" else "Sửa sự kiện") }, navigationIcon = { TextButton(onClick = onBack) { Text("Quay lại") } }) }) { innerPadding ->
        Column(Modifier.fillMaxSize().padding(innerPadding).verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(startTime, { startTime = it; errorMessage = null }, label = { Text("Bắt đầu (HH:mm)") }, modifier = Modifier.weight(1f), singleLine = true)
                OutlinedTextField(endTime, { endTime = it; errorMessage = null }, label = { Text("Kết thúc (HH:mm)") }, modifier = Modifier.weight(1f), singleLine = true)
            }
...
private fun EventTypeField(value: EventType, onValueChange: (EventType) -> Unit) {
    ...
    ExposedDropdownMenuBox(expanded, { expanded = !expanded }) {
        OutlinedTextField(value.toDisplayName(), {}, readOnly = true, label = { Text("Loại sự kiện") }, trailingIcon = { ExposedDropdownMenuDefaults.TrailingIcon(expanded) }, modifier = Modifier.fillMaxWidth().menuAnchor())
...
@Composable
private fun PriorityButton(value: Int, label: String, selected: Int, modifier: Modifier, onClick: () -> Unit) {
    if (value == selected) Button(onClick, modifier) { Text(label) } else OutlinedButton(onClick, modifier) { Text(label) }
}
