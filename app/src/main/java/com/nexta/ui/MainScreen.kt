...
@Composable
fun MainScreen(events: List<Event>, message: String? = null, onAddEvent: () -> Unit = {}, onDeleteEvent: (Event) -> Unit = {}) {
    ...
    LaunchedEffect(currentWeekOffset) {
        val target = (initialWeekPage + currentWeekOffset).coerceIn(0, TOTAL_WEEK_PAGES - 1)
    ...
    LaunchedEffect(weekPagerState.currentPage) {
        ...
        if (weekOffset != currentWeekOffset) {
            ...
            val targetPage = initialDayPage + ChronoUnit.DAYS.between(today, targetDate).toInt()
            dayPagerState.animateScrollToPage(targetPage.coerceIn(0, TOTAL_DAY_PAGES - 1))
        }
    }
    ...
    Scaffold(
        ...
        floatingActionButton = { FloatingActionButton(onClick = onAddEvent, containerColor = MaterialTheme.colorScheme.primaryContainer, contentColor = MaterialTheme.colorScheme.onPrimaryContainer) { Text("+", style = MaterialTheme.typography.headlineSmall) } }
    ) { innerPadding ->
        Column(modifier = Modifier.fillMaxSize().padding(innerPadding).padding(horizontal = 16.dp, vertical = 12.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            TextButton(onClick = { showDatePicker = true }, modifier = Modifier.fillMaxWidth(), contentPadding = androidx.compose.foundation.layout.PaddingValues(0.dp)) {
                Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(2.dp)) {
                    Text("${currentWeekStart.format(dateFormatter)} — ${currentWeekStart.plusDays(6).format(dateFormatter)}", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
            ...
            HorizontalPager(state = dayPagerState, modifier = Modifier.fillMaxWidth().weight(1f)) { page ->
                ...
                Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (dayEvents.isEmpty()) EmptyDayState(date == today)
                    else dayEvents.forEach { event -> EventCard(event, event.scheduleState(now), now) { eventToDelete = event } }
    ...
    if (showDatePicker) {
        val datePickerState = rememberDatePickerState(initialSelectedDateMillis = currentDate.toEpochMillis(), selectableDates = object : SelectableDates {
            override fun isSelectableDate(utcTimeMillis: Long): Boolean = utcTimeMillis.toLocalDate() in minSelectableDate..maxSelectableDate
        })
        DatePickerDialog(onDismissRequest = { showDatePicker = false }, confirmButton = {
            TextButton(onClick = {
                datePickerState.selectedDateMillis?.let { millis ->
                    ...
                    scope.launch { dayPagerState.animateScrollToPage(page.coerceIn(0, TOTAL_DAY_PAGES - 1)) }
                }
                showDatePicker = false
            }) { Text("Chọn") }
        }, dismissButton = { TextButton(onClick = { showDatePicker = false }) { Text("Hủy") } }) { DatePicker(state = datePickerState) }
    }

    eventToDelete?.let { event ->
        AlertDialog(onDismissRequest = { eventToDelete = null }, title = { Text("Xóa sự kiện?") }, text = { Text("Bạn có chắc muốn xóa \"${event.title}\" không?") }, confirmButton = { TextButton(onClick = { onDeleteEvent(event); eventToDelete = null }) { Text("Xóa") } }, dismissButton = { TextButton(onClick = { eventToDelete = null }) { Text("Hủy") } })
    }
}

@Composable
private fun WeekDayIndicator(weekPagerState: PagerState, initialWeekPage: Int, currentDate: LocalDate, today: LocalDate) {
    HorizontalPager(state = weekPagerState, modifier = Modifier.fillMaxWidth()) { page ->
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.Bottom) {
            weekDays.forEach { date ->
                ...
                Column(modifier = Modifier.width(44.dp).padding(horizontal = 2.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(2.dp)) {
                    ...
                    Text(date.dayOfMonth.toString(), style = MaterialTheme.typography.bodyMedium, fontWeight = if (isCurrent || isToday) FontWeight.Bold else FontWeight.Normal, color = if (isToday) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurface)
                    Surface(modifier = Modifier.size(if (isCurrent) 4.dp else 2.dp), shape = MaterialTheme.shapes.small, color = if (isCurrent || isToday) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.surfaceVariant) {}
                }
            }
        }
    }
...
private fun StatusDot(state: ScheduleState, active: Boolean) {
    val label = when (state) { ScheduleState.PAST -> "ĐÃ XONG"; ScheduleState.IN_PROGRESS -> "ĐANG HỌC"; ScheduleState.UPCOMING -> "SẮP TỚI" }
...
private fun countdownText(event: Event, state: ScheduleState, now: LocalDateTime): String = when (state) {
    ...
    ScheduleState.UPCOMING -> {
        val duration = Duration.between(now, event.startDateTime)
        if (duration.toDays() > COUNTDOWN_DAYS_LIMIT) "" else "Bắt đầu sau ${formatDuration(duration)}"
    }
...
private fun dayLabel(dayOfWeek: DayOfWeek): String = when (dayOfWeek) { DayOfWeek.MONDAY -> "T2"; DayOfWeek.TUESDAY -> "T3"; DayOfWeek.WEDNESDAY -> "T4"; DayOfWeek.THURSDAY -> "T5"; DayOfWeek.FRIDAY -> "T6"; DayOfWeek.SATURDAY -> "T7"; DayOfWeek.SUNDAY -> "CN" }
private fun vietnameseWeekday(dayOfWeek: DayOfWeek): String = when (dayOfWeek) { DayOfWeek.MONDAY -> "Thứ Hai"; DayOfWeek.TUESDAY -> "Thứ Ba"; DayOfWeek.WEDNESDAY -> "Thứ Tư"; DayOfWeek.THURSDAY -> "Thứ Năm"; DayOfWeek.FRIDAY -> "Thứ Sáu"; DayOfWeek.SATURDAY -> "Thứ Bảy"; DayOfWeek.SUNDAY -> "Chủ Nhật" }
...
private fun Event.scheduleState(now: LocalDateTime): ScheduleState = when { now < startDateTime -> ScheduleState.UPCOMING; now >= endDateTime -> ScheduleState.PAST; else -> ScheduleState.IN_PROGRESS }
