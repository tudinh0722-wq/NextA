...
class NextAWidgetProvider : AppWidgetProvider() {
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            ACTION_REFRESH, Intent.ACTION_TIME_CHANGED, Intent.ACTION_TIMEZONE_CHANGED, Intent.ACTION_DATE_CHANGED, Intent.ACTION_BOOT_COMPLETED -> refreshWidgets(context)
    ...
    companion object {
        ...
        private fun refreshWidgets(context: Context, widgetIds: IntArray? = null) {
            ...
            Thread {
                ...
                val cards = buildList<Pair<Event, Boolean>> { current?.let { add(it to true) }; upcoming.forEach { add(it to false) } }.take(2)
                ids.forEach { id -> updateWidget(manager, context, id, cards, now) }
                scheduleNextRefresh(context, current, upcoming.firstOrNull(), now)
            }.start()
        }

        private fun updateWidget(manager: AppWidgetManager, context: Context, widgetId: Int, cards: List<Pair<Event, Boolean>>, now: LocalDateTime) {
            ...
            val openAppIntent = PendingIntent.getActivity(context, OPEN_APP_REQUEST_BASE + widgetId, Intent(context, MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP }, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        ...
        private fun bindCard(views: RemoteViews, index: Int, card: Pair<Event, Boolean>?, now: LocalDateTime) {
            ...
            val progressContainer = if (index == 1) R.id.widget_progress_container_1 else R.id.widget_progress_container_2
            if (card == null) {
                views.setTextViewText(title, "Trống"); views.setTextViewText(startTime, ""); views.setTextViewText(endTime, ""); views.setTextViewText(countdown, "—"); views.setTextViewText(location, ""); views.setViewVisibility(note, View.GONE); views.setProgressBar(progress, 100, 0, false); views.setViewVisibility(progressContainer, View.GONE); views.setViewVisibility(progressDot, View.GONE); return
            }
            ...
            views.setTextViewText(countdown, formatCountdown(minutes, current))
            views.setTextViewText(location, event.location)
            if (current) {
                val totalMinutes = Duration.between(event.startDateTime, event.endDateTime).toMinutes().coerceAtLeast(1)
                val elapsedMinutes = Duration.between(event.startDateTime, now).toMinutes().coerceIn(0, totalMinutes)
                val progressPercent = ((elapsedMinutes * 100) / totalMinutes).toInt().coerceIn(0, 100)
        ...
        private fun scheduleNextRefresh(context: Context, current: Event?, next: Event?, now: LocalDateTime) {
            ...
            val target = when { boundary == null -> null; boundary.isBefore(minuteTick) -> boundary; else -> minOf(boundary, minuteTick) } ?: return
        ...
        private fun cancelRefresh(context: Context) = context.getSystemService(AlarmManager::class.java).cancel(refreshPendingIntent(context))
        private fun refreshPendingIntent(context: Context) = PendingIntent.getBroadcast(context, REQUEST_CODE, Intent(context, NextAWidgetProvider::class.java).apply { action = ACTION_REFRESH }, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        private fun formatCountdown(minutes: Long, current: Boolean): String {
            val value = when { minutes >= 24 * 60 -> "${minutes / (24 * 60)} ngày"; minutes >= 60 && minutes % 60 == 0L -> "${minutes / 60}h"; minutes >= 60 -> "${minutes / 60}h ${minutes % 60}m"; minutes > 0 -> "${minutes}m"; else -> "<1m" }
            return if (current) "Kết thúc sau\n$value" else "Bắt đầu sau\n$value"
        }
    }
}
