...
class NextAWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun scheduleNextRefresh(context: Context, current: Event?, next: Event?, now: LocalDateTime) {
            ...
            val relevant = Duration.between(now, boundary).toDays() <= COUNTDOWN_DAYS_LIMIT
            val target = if (relevant) maxOf(boundary, now.plusMinutes(1).withSecond(0).withNano(0)) else boundary
