...
- Selecting a recurring frequency expands an indented end section directly beneath it with two radio branches: `Kết thúc sau X ngày` and `Đến ngày X`.
- Both recurrence end branches are represented by a finite `until` boundary so the existing recurrence engine remains bounded without introducing a new recurrence table.
...
2. Verify bounded recurrence generation and series deletion on Samsung S23.
